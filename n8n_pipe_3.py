    ...
    async def pipe(
        ...
        if messages:
            ...
            try:
                ...
                session_id = chat_id
                if __metadata__:
                    task_body = __metadata__.get("task_body") or {}
                    metadata_chat_id = (
                        __metadata__.get("chat_id") or task_body.get("chat_id")
                    )
                    metadata_message_id = (
                        __metadata__.get("message_id") or task_body.get("message_id")
                    )
                    metadata_session_id = (
                        __metadata__.get("session_id") or task_body.get("session_id")
                    )

                    chat_id = chat_id or metadata_chat_id
                    message_id = message_id or metadata_message_id
                    session_id = metadata_session_id or chat_id or session_id
